...
async function generateWithKontextRefined(
  ...
  try {
    ...
    // Create hair-only mask from Kontext result using the independent test pipeline.
    console.log(`🎭 Creating Kontext result mask test from Stage 1 result...`);
    const kontextHairFaceMask = await createKontextResultMaskTest(kontextBase64, 30);
