...
/**
 * Create a user masked image showing EVERYTHING EXCEPT the hair
 * Result: Hair region replaced with gray (200,200,200), everything else shows original pixels
 * This tells FLUX which area to replace with the new hairstyle
...
async function callBiSeNetPipeline(
  imageBase64: string,
  mode: "hair_only" | "hair_only_simple" | "hair_only_ultra" | "kontext_result_mask_test" | "reference" | "reference_face_masked" | "facial_features_only" | "hair_with_skin_border",
  bufferPx: number,
    ...
    python.on('close', (code) => {
      ...
      try {
        ...
        if (output.success) {
          ...
          if (mode === "hair_only" || mode === "hair_only_simple" || mode === "hair_only_ultra" || mode === "kontext_result_mask_test") {
